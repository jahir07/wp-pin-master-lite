...
;(function ( $, window, document, undefined ) {
  ...
  // ------------------------------------------------------
  $.fn.PINMASTER_TAB_NAVIGATION = function() {
    return this.each(function() {

      var $this   = $(this),
          $nav    = $this.find('.pm-nav'),
          $reset  = $this.find('.pm-reset'),
          $expand = $this.find('.pm-expand-all');
      ...
      $nav.find('ul:first a').on('click', function (e) {
        ...
        if( $next.is('ul') ) {
          ...
          $el.closest('li').toggleClass('pm-tab-active');

        } else {

          $('#pm-tab-'+$target).show().siblings().hide();
          $nav.find('a').removeClass('pm-section-active');
          $el.addClass('pm-section-active');
          $reset.val($target);
      ...
      $expand.on('click', function (e) {
        e.preventDefault();
        $this.find('.pm-body').toggleClass('pm-show-all');
  ...
  // ======================================================
  // PINMASTER DEPENDENCY
  // ------------------------------------------------------
  $.PINMASTER.DEPENDENCY = function( el, param ) {
  ...
  $.fn.PINMASTER_DEPENDENCY = function ( param ) {
    return this.each(function () {
      new $.PINMASTER.DEPENDENCY( this, param );
    });
  };
  // ======================================================
  ...
  // ======================================================
  // PINMASTER IMAGE SELECTOR
  // ------------------------------------------------------
  $.fn.PINMASTER_IMAGE_SELECTOR = function() {
    return this.each(function() {
  ...
  // PINMASTER SORTER
  // ------------------------------------------------------
  $.fn.PINMASTER_SORTER = function() {
    return this.each(function() {

      var $this     = $(this),
          $enabled  = $this.find('.pm-enabled'),
          $disabled = $this.find('.pm-disabled');
      ...
      $enabled.sortable({
        ...
        update: function( event, ui ){
          ...
          if( ui.item.parent().hasClass('pm-enabled') ) {
  ...
  // ======================================================
  // PINMASTER MEDIA UPLOADER / UPLOAD
  // ------------------------------------------------------
  $.fn.PINMASTER_UPLOADER = function() {
    return this.each(function() {

      var $this  = $(this),
          $add   = $this.find('.pm-add'),
  ...
  // ======================================================
  // PINMASTER IMAGE UPLOADER
  // ------------------------------------------------------
  $.fn.PINMASTER_IMAGE_UPLOADER = function() {
    return this.each(function() {

      var $this    = $(this),
          $add     = $this.find('.pm-add'),
          $preview = $this.find('.pm-image-preview'),
          $remove  = $this.find('.pm-remove'),
          $input   = $this.find('input'),
          $img     = $this.find('img'),
          wp_media_frame;
  ...
  // ======================================================
  // PINMASTER IMAGE GALLERY
  // ------------------------------------------------------
  $.fn.PINMASTER_IMAGE_GALLERY = function() {
    return this.each(function() {

      var $this   = $(this),
          $edit   = $this.find('.pm-edit'),
          $remove = $this.find('.pm-remove'),
          $list   = $this.find('ul'),
          $input  = $this.find('input'),
          $img    = $this.find('img'),
      ...
      $this.on('click', '.pm-add, .pm-edit', function( e ) {

        var $el   = $(this),
            ids   = $input.val(),
            what  = ( $el.hasClass('pm-edit') ) ? 'edit' : 'add',
  ...
  // ======================================================
  // PINMASTER TYPOGRAPHY
  // ------------------------------------------------------
  $.fn.PINMASTER_TYPOGRAPHY = function() {
    return this.each( function() {

      var typography      = $(this),
          family_select   = typography.find('.pm-typo-family'),
          variants_select = typography.find('.pm-typo-variant'),
          typography_type = typography.find('.pm-typo-font');
  ...
  // PINMASTER GROUP
  // ------------------------------------------------------
  $.fn.PINMASTER_GROUP = function() {
    return this.each(function() {

      var _this           = $(this),
          field_groups    = _this.find('.pm-groups'),
          accordion_group = _this.find('.pm-accordion'),
          clone_group     = _this.find('.pm-group:first').clone();

      if ( accordion_group.length ) {
        accordion_group.accordion({
          header: '.pm-group-title',
          collapsible : true,
          active: false,
          animate: 250,
          heightStyle: 'content',
          ...
          beforeActivate: function( event, ui ) {
            $(ui.newPanel).PINMASTER_DEPENDENCY( 'sub' );
          }
        });
      }
      ...
      field_groups.sortable({
        axis: 'y',
        handle: '.pm-group-title',
        helper: 'original',
        cursor: 'move',
        placeholder: 'widget-placeholder',
        start: function( event, ui ) {
          var inside = ui.item.children('.pm-group-content');
        ...
        stop: function( event, ui ) {
          ui.item.children( '.pm-group-title' ).triggerHandler( 'focusout' );
          accordion_group.accordion({ active:false });
        }
      });

      var i = 0;
      $('.pm-add-group', _this).on('click', function( e ) {
        ...
        cloned.PINMASTER_DEPENDENCY( 'sub' );
        cloned.PINMASTER_RELOAD_PLUGINS();

        i++;

      });

      field_groups.on('click', '.pm-remove-group', function(e) {
        e.preventDefault();
        $(this).closest('.pm-group').remove();
      });
  ...
  // ======================================================
  // PINMASTER RESET CONFIRM
  // ------------------------------------------------------
  $.fn.PINMASTER_CONFIRM = function() {
    return this.each( function() {
  ...
  // ======================================================
  // PINMASTER SAVE OPTIONS
  // ------------------------------------------------------
  $.fn.PINMASTER_SAVE = function() {
    return this.each( function() {
  ...
  // ======================================================
  // PINMASTER SAVE TAXONOMY CLEAR FORM ELEMENTS
  // ------------------------------------------------------
  $.fn.PINMASTER_TAXONOMY = function() {
    return this.each( function() {
      ...
      if( $parent.attr('id') === 'addtag' ) {
        ...
        var $submit  = $parent.find('#submit'),
            ...
            $wrap    = $parent.find('.pm-framework'),
            $clone   = $wrap.find('.pm-element').clone(),
            $list    = $('#the-list'),
            flooding = false;
          ...
          if( !flooding ) {
            ...
            $list.on( 'DOMNodeInserted', function() {
              ...
              if( flooding ) {
                ...
                $wrap.PINMASTER_RELOAD_PLUGINS();
                $wrap.PINMASTER_DEPENDENCY();
  ...
  // ======================================================
  // PINMASTER UI DIALOG OVERLAY HELPER
  // ------------------------------------------------------
  ...
  // ======================================================
  // PINMASTER ICONS MANAGER
  // ------------------------------------------------------
  $.PINMASTER.ICONS_MANAGER = function() {
    ...
    base.init = function () {
      ...
      $wppml_body.on('click', '.pm-icon-add', function( e ) {
        ...
        var $this   = $(this),
            $dialog = $('#pm-icon-dialog'),
            $load   = $dialog.find('.pm-dialog-load'),
            $select = $dialog.find('.pm-dialog-select'),
            $insert = $dialog.find('.pm-dialog-insert'),
            $search = $dialog.find('.pm-icon-search');

        // set parent
        $parent = $this.closest('.pm-icon-select');
          ...
          $.ajax({
            ...
            success: function( content ) {
              ...
              $load.on('click', 'a', function( e ) {
                ...
                var icon = $(this).data('pm-icon');

                $parent.find('i').removeAttr('class').addClass(icon);
                $parent.find('input').val(icon).trigger('change');
                $parent.find('.pm-icon-preview').removeClass('hidden');
                $parent.find('.pm-icon-remove').removeClass('hidden');
                $dialog.dialog('close');
              ...
              $search.keyup( function(){
                ...
                $icons.each(function() {
                  ...
                  if ( $ico.data('pm-icon').search( new RegExp( value, 'i' ) ) < 0 ) {
                    $ico.hide();
                  } else {
                    $ico.show();
                  }
              ...
              $load.find('.pm-icon-tooltip').cstooltip({html:true, placement:'top', container:'body'});
      ...
      $wppml_body.on('click', '.pm-icon-remove', function( e ) {
        ...
        $parent.find('.pm-icon-preview').addClass('hidden');
  ...
  // ======================================================
  // PINMASTER SHORTCODE MANAGER
  // ------------------------------------------------------
  $.PINMASTER.SHORTCODE_MANAGER = function() {

    var base = this, deploy_atts;

    base.init = function () {

      var $dialog          = $('#pm-shortcode-dialog'),
          $insert          = $dialog.find('.pm-dialog-insert'),
          $shortcodeload   = $dialog.find('.pm-dialog-load'),
          $selector        = $dialog.find('.pm-dialog-select'),
      ...
      $wppml_body.on('click', '.pm-shortcode', function( e ) {
        ...
        // init chosen
        $selector.PINMASTER_CHOSEN();

        $shortcode_button = $(this);
        shortcode_target  = $shortcode_button.hasClass('pm-shortcode-textarea');
        ...
        if( shortcode_name.length ){
          ...
          $.ajax({
            ...
            success: function( content ) {
              ...
              shortcode_clone = $('.pm-shortcode-clone', $dialog).clone();

              $shortcodeload.PINMASTER_DEPENDENCY();
              $shortcodeload.PINMASTER_DEPENDENCY('sub');
              $shortcodeload.PINMASTER_RELOAD_PLUGINS();
      ...
      $insert.on('click', function ( e ) {
        ...
        switch ( shortcode_view ){
          ...
          case 'contents':
            ...
            $('[' + ruleAttr + ']', '.pm-dialog-load').each( function() {
          ...
          case 'clone':
            ...
            $('[' + ruleAttr + ']', '.pm-dialog-load .pm-element:not(.hidden)').each( function() {
            ...
            $('[' + cloneID + ']', '.pm-dialog-load').each( function() {
                ...
                $('[' + cloneAttr + ']', _this_clone.find('.pm-element').not('.hidden') ).each( function() {
      ...
      $dialog.on('click', '#shortcode-clone-button', function( e ) {
        ...
        $('.pm-shortcode-clone:last').after( cloned_el );

        // add - remove effects
        cloned_el.slideDown(100);

        cloned_el.find('.pm-remove-clone').show().on('click', function( e ) {
        ...
        // reloadPlugins
        cloned_el.PINMASTER_DEPENDENCY('sub');
        cloned_el.PINMASTER_RELOAD_PLUGINS();
        cloned++;
    ...
    base.validate_atts = function( _atts, _this ) {
      ...
      if( _this.data('check') !== undefined ) {
        el_value = _this.closest('.pm-element').find('input:checked').map( function() {
         return $(this).val();
        }).get();
      }
  ...
  $.PINMASTER.PARSE_COLOR_VALUE = function( val ) {
  ...
  $.fn.PINMASTER_COLORPICKER = function() {
    ...
    return this.each(function() {
      ...
      if( $this.data('rgba') !== false ) {
        ...
        // parse value
        var picker = $.PINMASTER.PARSE_COLOR_VALUE( $this.val() );
        ...
        $this.wpColorPicker({
          ...
          change: function( event, ui ) {
            ...
            $this.closest('.wp-picker-container').find('.pm-alpha-slider-offset').css('background-color', ui_color_value);
          ...
          create: function() {
            ...
            var a8cIris       = $this.data('a8cIris'),
                ...
                $alpha_wrap   = $('<div class="pm-alpha-wrap">' +
                                  '<div class="pm-alpha-slider"></div>' +
                                  '<div class="pm-alpha-slider-offset"></div>' +
                                  '<div class="pm-alpha-text"></div>' +
                                  '</div>').appendTo( $container.find('.wp-picker-holder') ),

                $alpha_slider = $alpha_wrap.find('.pm-alpha-slider'),
                $alpha_text   = $alpha_wrap.find('.pm-alpha-text'),
                $alpha_offset = $alpha_wrap.find('.pm-alpha-slider-offset');
            ...
            $alpha_slider.slider({
              ...
              create: function() {
                ...
                $container.on('click', '.wp-picker-default', function() {
                  ...
                  var default_picker = $.PINMASTER.PARSE_COLOR_VALUE( $this.data('default-color') ),
  ...
  // ------------------------------------------------------
  $.PINMASTER.WIDGET_RELOAD_PLUGINS = function() {
    $(document).on('widget-added widget-updated', function( event, $widget ) {
      $widget.PINMASTER_RELOAD_PLUGINS();
      $widget.PINMASTER_DEPENDENCY();
    });
  };
  ...
  // ------------------------------------------------------
  $.fn.PINMASTER_RELOAD_PLUGINS = function() {
    return this.each(function() {
      $('.chosen', this).PINMASTER_CHOSEN();
      $('.pm-field-image-select', this).PINMASTER_IMAGE_SELECTOR();
      $('.pm-field-image', this).PINMASTER_IMAGE_UPLOADER();
      $('.pm-field-gallery', this).PINMASTER_IMAGE_GALLERY();
      $('.pm-field-sorter', this).PINMASTER_SORTER();
      $('.pm-field-upload', this).PINMASTER_UPLOADER();
      $('.pm-field-typography', this).PINMASTER_TYPOGRAPHY();
      $('.pm-field-color-picker', this).PINMASTER_COLORPICKER();
      $('.pm-help', this).PINMASTER_TOOLTIP();
    });
  };
  ...
  $(document).ready( function() {
    $('.pm-framework').PINMASTER_TAB_NAVIGATION();
    $('.pm-reset-confirm, .pm-import-backup').PINMASTER_CONFIRM();
    $('.pm-content, .wp-customizer, .widget-content, .pm-taxonomy').PINMASTER_DEPENDENCY();
    $('.pm-field-group').PINMASTER_GROUP();
    $('.pm-save').PINMASTER_SAVE();
    $('.pm-taxonomy').PINMASTER_TAXONOMY();
    $('.pm-framework, #widgets-right').PINMASTER_RELOAD_PLUGINS();
    $.PINMASTER.ICONS_MANAGER();
    $.PINMASTER.SHORTCODE_MANAGER();
    $.PINMASTER.WIDGET_RELOAD_PLUGINS();
  });
