...
abstract class PinMaster_Options extends PinMaster_Abstract {
  ...
  public function element_after() {
    ...
    $out  = ( isset( $this->field['info'] ) ) ? '<p class="pm-text-desc">'. $this->field['info'] .'</p>' : '';
  ...
  public function element_debug() {
    ...
    if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'PM_OPTIONS_DEBUG' ) && PM_OPTIONS_DEBUG ) ) {
      ...
      $out .= "<pre>";
      $out .= "<strong>". esc_html__( 'CONFIG', 'pin-master' ) .":</strong>";
      $out .= "\n";
      ob_start();
      ...
      if( ! empty( $value ) ) {
        $out .= "\n\n";
        $out .= "<strong>". esc_html__( 'VALUE', 'pin-master' ) .":</strong>";
        $out .= "\n";
        ob_start();
        var_export( $value );
        $out .= htmlspecialchars( ob_get_clean() );
  ...
  public function element_help() {
    return ( isset( $this->field['help'] ) ) ? '<span class="pm-help" data-title="'. $this->field['help'] .'"><span class="fa fa-question-circle"></span></span>' : '';
  }
  ...
  public function element_after_multilang() {
    ...
    if ( is_array( $this->multilang ) ) {
      ...
      $out .= '</fieldset>';
      $out .= '<p class="pm-text-desc">'. sprintf( esc_html__( 'You are editing language: ( %s )', 'pin-master' ), '<strong>'. $this->multilang['current'] .'</strong>' ) .'</p>';
